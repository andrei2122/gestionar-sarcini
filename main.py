...
# ---------------------------------------------------------------------------
# Configurare (citita din variabile de mediu / fisierul .env)
# ---------------------------------------------------------------------------

load_dotenv()  # citeste .env daca exista; in productie variabilele vin din platforma
...
# CORS ramane (optional cand frontend-ul e servit din acelasi server, dar nu strica)
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5500",
...
# ---------------------------------------------------------------------------
# Endpoint health check (bonus) - nu necesita autentificare
# ---------------------------------------------------------------------------
...
def finalizeaza_sarcina(
    ...
    db.commit()
    return dict(
        db.execute("SELECT * FROM sarcini WHERE id = ?", (sarcina_id,)).fetchone()
    )
...
# ---------------------------------------------------------------------------
# ULTIMUL RAND: servirea frontend-ului din static/ (dupa toate endpoint-urile)
# ---------------------------------------------------------------------------
